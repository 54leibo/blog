...
| 分支名称 | 来源分支 | 合并目标分支 | 环境绑定 | 说明 |
|---------|---------|------------|---------|------|
| prod | - | - | 生产环境 | 稳定分支，随时可部署 |
| test | prod | - | 测试环境 | 用于测试环境验证，独立运行 |
| dev | prod | - | 开发环境 | 用于开发环境调试，独立运行 |
| feat/* | prod | dev, test, prod | - | 功能开发分支，完成后需向各环境分支合并 |
| hotfix/* | prod | prod | - | 紧急修复分支，直接修复生产问题 |
...
| 分支名称 | 来源分支 | 合并目标分支 | 环境绑定 | 说明 |
|---------|---------|------------|---------|------|
| main | - | - | 生产环境 | 稳定分支，保存已发布的版本 |
| develop | main | - | 开发环境 | 开发集成分支，包含最新功能 |
| feature/* | develop | develop | - | 功能开发分支，完成后合并回develop |
| release/* | develop | develop, main | 预发布环境 | 版本发布准备分支，修复后同时合并到develop和main |
| hotfix/* | main | main, develop | - | 紧急修复分支，修复生产问题后合并到main和develop |
...
| 分支名称 | 来源分支 | 合并目标分支 | 环境绑定 | 说明 |
|---------|---------|------------|---------|------|
| main | - | - | 生产环境 | 主分支，始终保持可部署状态 |
| feature/* | main | main | - | 功能开发分支，通过PR合并回main |
...
| 分支名称 | 来源分支 | 合并目标分支 | 环境绑定 | 说明 |
|---------|---------|------------|---------|------|
| production | - | - | 生产环境 | 生产环境分支，反映当前部署状态 |
| pre-production | production | production | 预发布环境 | 预发布环境分支，用于最终验证 |
| dev | pre-production | pre-production | 开发环境 | 开发集成分支，包含已完成功能 |
| feature/* | dev | dev | - | 功能开发分支，完成后合并到dev |
...
| 分支名称 | 来源分支 | 合并目标分支 | 环境绑定 | 说明 |
|---------|---------|------------|---------|------|
| trunk/main | - | - | 生产环境 | 主干分支，所有开发都基于此分支 |
| release/* | trunk | - | 生产环境 | 可选的发布分支，用于版本发布 |
| feature/* | trunk | trunk | - | 短生命周期功能分支（通常<1天），快速合并回主干 |
